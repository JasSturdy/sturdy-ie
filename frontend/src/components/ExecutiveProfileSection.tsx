  ...
  return (
    <div className="mt-12 md:mt-24 overflow-hidden bg-black">
      ...
      <div className="px-6 md:px-10 lg:px-16 mb-8 md:mb-12">
        <h2 className="text-3xl font-light text-[#c5f018] md:text-4xl lg:text-5xl">
          Executive <span className="text-white">Profile</span>
        </h2>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2">
        ...
        <div className="lg:order-1 flex flex-col justify-between px-6 py-8 md:px-10 md:py-10 lg:px-16 lg:py-12">
          <div className="bg-[#222222] p-6 sm:p-8 md:p-10 lg:p-14">
            <div
              key={current.id}
              className="animate-in fade-in slide-in-from-left-4 duration-500 space-y-4 text-sm leading-relaxed text-zinc-300 md:text-base"
            >
              {current.paragraphs.map((para, i) => (
                <p
                  key={i}
                  className={para.isBold ? "font-bold text-white" : ""}
                >
                  {para.text}
                </p>
              ))}
