...
export const CATEGORY_STYLES: Record<
  string,
  { active: string; badge: string; tag: string }
> = {
  All: {
    active: "bg-[#c5f018] border-[#c5f018] text-black",
  ...
  "Governance & Compliance": {
    ...
    tag:    "border-blue-500/40 text-blue-400 bg-blue-500/10",
  },
  "Research Collaboration": {
    active: "bg-emerald-500 border-emerald-500 text-white",
    badge:  "bg-white/20 text-white",
    tag:    "border-emerald-500/40 text-emerald-400 bg-emerald-500/10",
  },
  "Interoperability & Standards": {
    active: "bg-violet-500 border-violet-500 text-white",
    badge:  "bg-white/20 text-white",
    tag:    "border-violet-500/40 text-violet-400 bg-violet-500/10",
  },
...
function mapDoc(a: any): MyInsightDetail {
  return {
    slug:     a.slug,
    title:    a.title,
    category: a.category,
...
export async function getMyInsightsIndex(): Promise<MyInsightIndex[]> {
  try {
    ...
    return docs.map((a: any) => {
      const full = mapDoc(a);
      const { summary: _s, sections: _sec, ...index } = full;
      return index as MyInsightIndex;
    });
  } catch {
    return [];
  }
...
export async function getMyInsightBySlug(
  slug: string
): Promise<MyInsightDetail | null> {
  try {
    const url = `${API_URL}/api/myinsights?where[slug][equals]=${encodeURIComponent(slug)}&depth=1&limit=1`;
    const res = await fetch(url, { cache: "no-store" });
